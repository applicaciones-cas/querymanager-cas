...
import ph.com.guanzongroup.login.LoginDialog;
import ph.com.guanzongroup.login.LoginResult;
import ph.com.guanzongroup.querymanager.views.MainInterfaceController;
...
public class Login extends Application {
    ...
    public void start(Stage stage) throws Exception {    
        //call login modal
        LoginResult login = LoginDialog.show("gRider", stage);
        ...
        //log user on the main app
        GRiderCAS poGRider = new GRiderCAS(login.getProductId());
        if (!poGRider.loadEnv(login.getProductId())){
            System.out.println(poGRider.getMessage() + poGRider.getMessage());
            System.exit(0);
        }
        if (!poGRider.loadUser(login.getProductId(), login.getUserId())){
            System.out.println(poGRider.getMessage() + poGRider.getMessage());
            System.exit(0);
        }
